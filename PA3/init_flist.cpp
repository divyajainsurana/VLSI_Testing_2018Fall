...
void ATPG::generate_fault_list(void) {
 ...
 int total_detect_num=0;
  ...
  for (auto pos = sort_wlist.crbegin(); pos != sort_wlist.crend(); ++pos) {
    ...
    /* for  NOR NOT BUF, their GI fault is equivalent to GO STR fault */
    switch (n->type) {
            case   NOT:
      case   BUF:
        f->eqv_fault_num = 1;
    ...
    f->fault_type = STF;
    f->to_swlist = w->wlist_index;
    /* for  NOT BUF, their GI fault is equivalent to GO STF fault */
    switch (n->type) {
          case   NOT:
      case   BUF:
        f->eqv_fault_num = 1;
    ...
    if (w->onode.size() > 1) {
	  ...
	  case AND:
        ...
        /* create STF for AND NAND EQV XOR gate inputs  */
        switch (nptr_ele->type) {
          case OUTPUT:
          case   AND:
          case  NAND: 
...
void ATPG::compute_fault_coverage(void) {
  ...
  int no_of_detect,eqv_no_of_detect,eqv_num_of_gate_fault,num_of_gate_fault,fault_num;
  fptr f;
 ...
 // fprintf(stdout,"#number of transition delay faults = %d\n",eqv_num_of_gate_fault);
