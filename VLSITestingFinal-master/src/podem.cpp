...
vector<ATPG::Decision> ATPG::null_decisions;

/* generates a single pattern for a single fault */
int ATPG::podem(const fptr fault, int& current_backtracks, const vector<int> &constraints, bool activate_only, vector<Decision> &saved_decisions ) {
  ...
  if( !constraints.empty() )
    podem_setup_constraints( constraints );
  // end setup input constraints

  // setup decision tree
  if( !saved_decisions.empty() )
  {
    assert( saved_decisions.size() <= cktin.size() );

    for( const Decision &decision : saved_decisions )
    {
       wptr wire = decision.pi;

       if( wire->value == U )
       {
         wire->value  =   decision.value;
         wire->flag   |=  CHANGED;
       }
       else if( wire->value != decision.value )
       {
         return FALSE;
       }
       if( decision.all_assigned )
         wire->flag |= ALL_ASSIGNED;
    }
    for( auto it = saved_decisions.crbegin() ; it != saved_decisions.crend() ; ++it )
       decision_tree.push_front( it->pi );

    wpi = nullptr;

    while( !decision_tree.empty() && ( wpi == nullptr ) )
    {
      // if both 01 already tried, backtrack.
      ...
      // else, flip last decision, flag ALL_ASSIGNED.
      else
      {
    ...
    if (wpi == nullptr) return FALSE;
    sim();
  }
  // end setup decision tree
  ...
  while ((no_of_backtracks < backtrack_limit) && !no_test &&
    ...
    if( activate_only && fault_activated( fault ) ) return TRUE;
...
again:  if (wpi) {
  ...
  // save decision tree
  if( &saved_decisions != &null_decisions )
  {
    saved_decisions.clear();

    for( wptr wire : decision_tree )
       saved_decisions.push_back( { wire, wire->value, static_cast<bool>( wire->flag & ALL_ASSIGNED ) } );
  }
  // end save decision tree
...
bool ATPG::fault_activated( const fptr fault )
{
  wptr wire;

  switch( fault->io )
  {
    case GI: wire = fault->node->iwire[fault->index];
    case GO: wire = fault->node->owire.front();
    default: return false;
  }

  switch( fault->fault_type )
  {
    case STUCK0:  return ( wire->value == TRUE  );
    case STUCK1:  return ( wire->value == FALSE );
    default:      return false;
  }
}

void ATPG::podem_setup_constraints( const vector<int> &constraints )
{
  assert( constraints.size() == cktin.size() );

  for( size_t i = 0 ; i < cktin.size() ; ++i )
  {
     if( constraints[i] == U ) continue;

     cktin[i]->value =  constraints[i];
