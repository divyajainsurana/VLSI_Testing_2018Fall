...
int useful = FALSE;
  ...
  for (i = vectors.size()-1; i >= 0; i--) {
    tdfault_sim_a_vector(vectors[i], current_detect_num,useful);
    total_detect_num += current_detect_num;
    fprintf(stdout,"vector[%d] detects %d faults (%d)\n",i,current_detect_num,total_detect_num);
  }
}// fault_simulate_vectors

void ATPG::tdfault_sim_a_vector(const string& vec, int& num_of_current_detect, int& useful, const bool& fdrop) {
  size_t i, nckt;
  fptr f;
  ...
  tdfault_sim_a_vector2(vec, num_of_current_detect, useful, fdrop);


}

/* fault simulate a single test vector */
void ATPG::tdfault_sim_a_vector2(const string& vec, int& num_of_current_detect, int& useful, const bool& fdrop) {
  wptr w,faulty_wire;
  /* array of 16 fptrs, which points to the 16 faults in a simulation packet  */
  fptr simulated_fault_list[num_of_pattern];
  fptr f;
  int fault_type;
  size_t i,start_wire_index, nckt;
  size_t num_of_fault;
      ...
      for (nptr nptr_ele: w->onode) {
        ...
        switch (nptr_ele->type) {
          ...
          case    XOR:
            ...
            for (size_t k = 0; k < nptr_ele->iwire.size(); k++) {
