...
/* parse the input switches & arguments */
  while(i< argc) {
    if (strcmp(argv[i],"-anum") == 0) {
      ...
      atpg.setRandSeed(atoi(argv[i+1]));
      i+=2;
    }
    else if (strcmp(argv[i],"-tdfatpg") == 0) {
      atpg.set_perform_tdf_atpg(true);
      ++i;
    }
    else if (strcmp(argv[i],"-compression") ==0){
      atpg.set_perform_compress(true);
      ++i;
    }
    else if (strcmp(argv[i],"-stc") ==0){
      atpg.set_perform_STC(true);
      ++i;
    }
    else if (strcmp(argv[i],"-dtc") ==0){
      atpg.set_perform_DTC(true);
      ++i;
    }
    else if (argv[i][0] == '-') {
      j = 1;
      while (argv[i][j] != '\0') {
        if (argv[i][j] == 'd') {
          ...
          fprintf(stderr, "atpg: unknown option \"%s\"\n", argv[i]);
          usage();
        }
      }
      i++ ;
...
/* if vector file is provided, read it */
  ...
  if( atpg.get_tdfsim_only() || atpg.get_perform_tdf_atpg() )
...
void usage(void) {
   ...
   fprintf(stderr, "    -fsim <filename>: STF simulation only; filename provides vectors\n");
   fprintf(stderr, "    -tdfsim <filename>: TDF simulation only; filename provides vectors\n");
   fprintf(stderr, "    -anum <num>: <num> specifies number of vectors per fault\n");
   fprintf(stderr, "    -ndet <num>: <num> specifies number of detedction per fault\n");
   fprintf(stderr, "    -compression: preform test compression\n");
   fprintf(stderr, "    -bt <num>: <num> specifies number of backtracks\n");
   fprintf(stderr, "    -rs <num>: <num> specifies the random seed\n");
...
void ATPG::set_perform_tdf_atpg(const bool& b) {
  this->perform_tdf_atpg = b;
}

void ATPG::set_perform_compress(const bool& b){
  this->perform_STC = b;
  this->perform_DTC = b;
}

void ATPG::set_perform_STC(const bool& b){
  this->perform_STC = b;
}

void ATPG::set_perform_DTC(const bool& b){
  this->perform_DTC = b;
}
...
void ATPG::setRandSeed(const int& i) {
    srand(i);
}
void ATPG::setExecFilePath(const char* s) {
    this->execFilePath = string(s);
}